...
router.post('/', async (req, res) => {
  try {
    ...
    if (childContext && childContext.trim().length > 0) {
      finalSystemPrompt += `\n\n**COMPLETE FAMILY INFORMATION - ONLY USE EXACT DETAILS PROVIDED:**\n${childContext}\n\n**CRITICAL ANTI-ASSUMPTION RULES:**\n- These family members are REAL and known to you from previous conversations\n- ONLY mention details explicitly provided in the family information above\n- DO NOT assume, invent, or guess ANY details not specifically listed (hobbies, interests, activities, personality traits, etc.)\n- If information is missing (like interests, school details, activities), say "I'd love to learn more about [specific topic]" instead of guessing\n- Use ONLY the exact names, ages, genders, relationships, and medical information provided\n- Ask specific questions to learn MORE details rather than making assumptions\n- When referencing family members, stick to only the facts provided in the context\n- If you don't know something about a family member, admit it and ask to learn more`;
      console.log(`✅ Added STRICT no-assumption family context with ${(childContext.match(/\*\*[^*]+\*\*\n- Relationship:/g) || []).length} family members`);
      console.log(`🎯 System prompt includes anti-assumption rules: ${finalSystemPrompt.includes('CRITICAL ANTI-ASSUMPTION') ? 'YES' : 'NO'}`);
    } else {
